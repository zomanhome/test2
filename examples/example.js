(function() {
  ...
  angular
    ...
    .controller(
      ...
      sessionLimitListController
    );

  sessionLimitListController.$inject = [
    "$log",
    "$scope",
    "SessionLimitListModel",
  ...
  function sessionLimitListController(
    $log,
    $scope,
    SessionLimitListModel,
    sessionLimitsListApi,
    ...
    function _setFormsWatchers(data) {
      data.forEach(function(row) {
        sessionLimitListVm.watchers.push(
          $scope.$watch(
            "sessionLimitListVm.forms[" + row.id + "].$dirty",
            _updateEditStatus
          )
        );
        ...
        sessionLimitListVm.watchers.push(
          $scope.$watch(
            "sessionLimitListVm.forms[" + row.id + "].$invalid",
            function(newValue) {
              if (newValue) {
                sessionLimitListVm.areAllFormsValid = false;
              } else {
                sessionLimitListVm.areAllFormsValid = Object.values(
                  sessionLimitListVm.forms
                ).every(function(form) {
    ...
    $scope.$on("cancelSessionLimitEdit", function() {
      var data = {
        ...
        impactMessage: lcI18n.getLocalizedValue(
          "lcCommon.recordDiscardMessageOne"
        ),
        confirmationMessage: lcI18n.getLocalizedValue(
          "lcCommon.recordDiscardMessageTwo"
        )
      };

      Dialog.summonModal(
        "partials/global/_discard-configuration.html",
        data
      ).then(function() {
        sessionLimitListVm.listViewModel.rowCollection = angular.copy(
          sessionLimitListVm.initialData
        );
