describe("user.management.sessionLimitList.controller", function() {
  ...
  beforeEach(function() {
    this.mockSessionLimitsListApi = jasmine.createSpyObj(
      "sessionLimitsListApi",
      ["getAllSessionLimits", "updateSessionLimits"]
    );
    this.mockDialog = jasmine.createSpyObj("Dialog", ["summonModal"]);
    this.mockUserManagementService = jasmine.createSpyObj(
      "userManagementService",
      ["setEditStatus", "setSaveEnableStatus"]
    );
  });
  ...
  beforeEach(inject(function($controller, $rootScope, $q, $timeout) {
    ...
    this.mockSessionLimitsListApi.getAllSessionLimits.and.returnValue(
      this.getAllSessionLimitsDefer.promise
    );

    this.sessionLimitListController = $controller(
      "user.management.sessionLimitList.controller",
      {
        $scope: this.scope,
  ...
  describe(".init()", function() {
    ...
    it("Should  call getAllSessionLimits if Session Limit controller init", function() {
      expect(
        this.mockSessionLimitsListApi.getAllSessionLimits
      ).toHaveBeenCalled();
    });
    ...
    describe("when sessionLimitsListApi.getAllSessionLimits resolves with data", function() {
      beforeEach(function() {
        ...
        this.sessionLimitListController = this.ctrl(
          "user.management.sessionLimitList.controller",
          {
            $scope: this.scope,
            sessionLimitsListApi: this.mockSessionLimitsListApi,
      ...
      it("initializes the row data appropriately", function() {
        expect(
          this.sessionLimitListController.listViewModel.rowCollection
        ).toEqual(this.sessionLimitsData.body);
      });

      it("initializes initialData appropriately", function() {
        expect(this.sessionLimitListController.initialData).toEqual(
          this.sessionLimitsData.body
        );
      });
  ...
  describe("when the 'cancelSessionLimitEdit' event is called", function() {
    beforeEach(function() {
      ...
      this.mockDialog.summonModal.and.returnValue(
        this.discardConfigurationModalDefer.promise
      );
    ...
    describe("when the modal is confirmed", function() {
      ...
      it("calls userManagementService.setEditStatus with false", function() {
        expect(
          this.mockUserManagementService.setEditStatus
  ...
  describe("when the 'saveSessionLimits' event is called", function() {
    beforeEach(function() {
      this.getAllSessionLimitsDefer = this.q.defer();
      this.mockSessionLimitsListApi.updateSessionLimits.and.returnValue(
        this.getAllSessionLimitsDefer.promise
      );
    ...
    it("displays the discard configuration modal", function() {
      expect(
        this.mockSessionLimitsListApi.updateSessionLimits
      ).toHaveBeenCalled();
    });
